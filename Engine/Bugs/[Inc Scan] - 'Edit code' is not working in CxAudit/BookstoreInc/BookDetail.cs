...
Rating_BeforeSQLExecute(sSQL,"Update");
		OleDbCommand cmd = new OleDbCommand(sSQL, Utility.Connection);
			try {
				cmd.Parameters.Add("@rating", OleDbType.VarChar, 80).Value = Rating_rating.SelectedItem.Value;
				cmd.Parameters.Add("@itemId", OleDbType.Integer).Value =  Rating_item_id.Value;
				cmd.ExecuteNonQuery();
			} catch(Exception e) {
